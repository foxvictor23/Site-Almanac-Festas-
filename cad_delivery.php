...
<div class='boxadm'><i class="fa fa-folder-open"></i> Cadastrar Delivery</div>
<br/>
...
$files = $_FILES['imagem'];
if($files == ''){
echo '<script>alert("Imagem obrigatória!");location.href="cad_delivery.php"</script>';
}elseif($files['error'] == '4'){
echo '<script>alert("Imagem obrigatória!");location.href="cad_delivery.php"</script>';
}else{
if($painel->Cadtb_delivery($id_cat, $titulo, $slug, $cidade, $categoria, $texto, $files['tmp_name'], $files['name'])){
echo '<script>alert("Cadastrado!");location.href="cad_delivery.php"</script>';
}else{
echo '<script>alert("Erro ao cadastrar, verifique a conexão com a internet.");location.href="cad_delivery.php"</script>';
}
}
}
?>
